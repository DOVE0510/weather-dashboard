...
function toggleManualMode() {
    ...
    const ledPowerSwitch = document.getElementById("ledPowerSwitch");

    if (manualMode) {
        ledPowerSwitch.disabled = false;
        ledPowerSwitch.checked = ledState;
    } else {
        ledPowerSwitch.disabled = true;
        updateAutoLED();
    }
...
function updateAutoLED() {
    ...
    if (weatherData.light < LUX_THRESHOLD && !ledState) {
        client.publish(LED_TOPIC, "1");
        ledState = true;
    }
    else if (weatherData.light >= LUX_THRESHOLD && ledState) {
